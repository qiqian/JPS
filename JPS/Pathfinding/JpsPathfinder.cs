...
///
/// 这样：阻挡改动后无需重建任何表（O(1) 置脏），寻路时“用到哪格才更新哪格”，
/// 跨多次查询（两次阻挡变化之间）可复用已洗白的正交跳点，明显快于纯经典逐格。
///
/// 性能手法同前：逐节点数据用扁平数组（无哈希）+ 代次戳免清零 + 缓冲区复用，全程整数运算。
/// </summary>
public sealed class JpsPathfinder
{
    ...
    private int[] _cardGen = [];    // 每个表项的世代戳
    private int _cardValidGen;      // 当前有效世代；与下面的版本不符即整体失效
    private int _cardMapVersion = -1;

    // 当前查询用的“可走”委托（= map.IsWalkable）
    ...
    public PathResult FindPath(GridMap map, bool collectDebug = true)
    {
        ...
        // 阻挡布局变化 → O(1) 整体置脏惰性跳点 memo
        if (_cardMapVersion != map.Version)
        {
            _cardValidGen++;
            if (_cardValidGen == int.MaxValue)
            {
                Array.Clear(_cardGen);
                _cardValidGen = 1;
            }
            _cardMapVersion = map.Version;
        }
    ...
    /// <summary>
    /// 取 (x,y) 沿正交方向 (dx,dy) 的带符号跳点距离（&gt;0 跳点、&lt;=0 到墙）。
    /// 命中 clean 直接读；未命中沿射线扫一次并把整段 run 一起洗白。
    /// </summary>
    private int CardinalDist(GridMap map, int x, int y, int dx, int dy, int dir)
    {
        int idx0 = ((y * _w + x) * 4) + dir;
        if (_cardGen[idx0] == _cardValidGen)
            return _cardDist[idx0];
        ...
        while (true)
        {
            ...
            ry += dy;
            s++;
            if (!map.IsWalkable(rx, ry)) { jumpFound = false; break; }            // 第 s 步是墙/越界
            if (JpsRules.IsJumpPoint(_walk, rx, ry, dx, dy)) { jumpFound = true; break; }   // 第 s 步是跳点
        }

        // 回填整段 run（步 k=0..s-1 的可走格），它们到 J/墙 的距离可由 k 推出
        int fx = x, fy = y;
        for (int k = 0; k <= s - 1; k++)
        {
            int fi = ((fy * _w + fx) * 4) + dir;
            _cardDist[fi] = jumpFound ? (s - k) : -((s - 1) - k);
            _cardGen[fi] = _cardValidGen;
            fx += dx;
            fy += dy;
        }
    ...
    private JumpEntry CardinalJump(GridMap map, int x, int y, int dx, int dy, int gx, int gy)
    {
        ...
        int dist = CardinalDist(map, x, y, dx, dy, dir);
    ...
    private void EnsureBuffers(GridMap map)
    {
        ...
        _cardGen = new int[_size * 4];
        _cardValidGen = 0;
        _cardMapVersion = -1;   // 强制下次重新置脏
    }
